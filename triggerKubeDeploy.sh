...
curl -X POST -H "$CRUMB" "https://$JENKINS_USER_CREDS@$JENKINS_HOSTNAME/job/$SERVICE_NAME/buildWithParameters?TOKEN_NAME=$JENKINS_TRIGGER_TOKEN&BRANCH=$TRAVIS_BRANCH&IMAGE_TAG=$IMAGE_TAG&AWS_ACCOUNT_ID=$AWS_ACCOUNT_ID"
